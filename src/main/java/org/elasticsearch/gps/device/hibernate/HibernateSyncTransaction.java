 ...
 * @author kimchy
 */
public class HibernateSyncTransaction // (AGR_OSEM) ... extends AbstractTransaction
{
    ...
    public HibernateSyncTransaction(SessionFactory sessionFactory, boolean commitBeforeCompletion, TransactionFactory transactionFactory) {
        // (AGR_OSEM) ... super(transactionFactory);
    ...
    public void begin(InternalCompassSession session) throws CompassException {
		...
		// (AGR_OSEM) ... session.getSearchEngine().begin();
	    ...
	    // (AGR_OSEM) ... transaction.registerSynchronization(new HibernateTransactionSynchronization(session, transaction, newTransaction, commitBeforeCompletion, transactionFactory));
        }
	catch (Exception e) {
            throw new TransactionException("Begin failed with exception", e);
        }

	// (AGR_OSEM) ... setBegun(true);
    }
    ...
    public boolean wasRolledBack() throws TransactionException {
        ...
        // (AGR_OSEM) ... if (!isBegun())		return false;
    ...
    public boolean wasCommitted() throws TransactionException {
        ...
        // (AGR_OSEM) ... if (!isBegun() || commitFailed)		return false;
    ...
    private static class HibernateTransactionSynchronization implements Synchronization {
	    ...
	    // (AGR_OSEM) ... session.getSearchEngine().commit(true);
        }
        ...
        public void afterCompletion(int status) {
			...
			// (AGR_OSEM) ... session.getSearchEngine().rollback();
                    }
                }
            } catch (Exception e) {
                // TODO swallow??????
                ...
                // (AGR_OSEM) ... ((HibernateSyncTransactionFactory) transactionFactory).unbindSessionFromTransaction(tx, session);
